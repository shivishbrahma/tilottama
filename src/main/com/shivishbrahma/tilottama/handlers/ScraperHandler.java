...
public class ScraperHandler {
     ...
     * @return
     */
    public Arg findArg(Service s, String name) {
        List<Arg> argList = s.getConfig().getArgs().stream().filter(p -> p.getName().equalsIgnoreCase(name))
                .collect(Collectors.toList());
        if (argList.size() > 0) {
    ...
    public JsonElement getJsonRequest(Service s, String args) {
        ...
        try {
            ...
            url = String.format(s.getConfig().getUrl(), s.getConfig().getKey()) + url;
