 ...
 * 
 */
package com.shivishbrahma.tilottama.gui;
...
import com.shivishbrahma.tilottama.models.Currency;
import com.shivishbrahma.tilottama.models.Temperature;
import com.shivishbrahma.tilottama.par.CurrencyHandler;
import com.shivishbrahma.tilottama.util.CurrencyConvertor;
...
public class Converter extends Gui {
		...
		tempConv = new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (fromUnit.equalsIgnoreCase(toUnit)) {
					...
					fromVal.setText(Temperature.temperatureFormat(fromUnit.charAt(0), fromValue));
					toVal.setText(Temperature.temperatureFormat(toUnit.charAt(0), fromValue));
				} else {
					double toValue = Temperature.temperatureConv(getCharTemperature(fromUnit),
							getCharTemperature(toUnit), fromValue);
					toText.setText(String.format("%.3f", toValue));
					fromVal.setText(Temperature.temperatureFormat(fromUnit.charAt(0), fromValue));
					toVal.setText(Temperature.temperatureFormat(toUnit.charAt(0), toValue));
				}
			}
		};
	...
	public void initCurrency() {
		List<Currency> currList = CurrencyHandler.getCurrencies();
		fromSelect.removeAllItems();
